...
export default function Games() {
  ...
  useEffect(() => {
    if (router.query?.selected) {
      ...
      const newPlayersList = selectedPlayerQS.map(
        (index) => playersList[index]
      );
  ...
  useEffect(() => {
    if (selectedPlayers.length) {
      const result = runGenetic(20, selectedPlayers, 100, 0.7, 0.2, 0.1, true);
  ...
  // Auxiliar functions
  const calculateTeamScore = (team: number[]) =>
    team.reduce((acc, val) => acc + selectedPlayers[val].score, 0);
  ...
  return (
    <div className="container">
      ...
      <main>
        ...
        <p>
          Max Possible Score: <b>{solution?.maxPossibleScore}</b>
        </p>
        <p>
          The best combination got: <b>{solution?.maxScore}</b>
        </p>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {teams?.map((teamsOption, index) => {
              return (
                <tr key={index}>
                  <td>
                    {teamsOption.population
                      .slice(0, teamSize)
                      .map((indexPlayer) => selectedPlayers[indexPlayer].name)
                      .join(',')}
                  </td>
                  <td>
                    {calculateTeamScore(
                      teamsOption.population.slice(0, teamSize)
                    )}
                  </td>
                  <td>
                    {teamsOption.population
                  ...
                  <td>
                    {calculateTeamScore(teamsOption.population.slice(teamSize))}
                  </td>
                </tr>
              );
            })}
